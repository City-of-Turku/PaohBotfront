...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        ...
        async 'axios.requestWithJsonBody'(url, method, data, authToken = null) {
            let loggedData = data;
            ...
            check(data, Object);
            try {
                const axiosJson = axios.create({ params: { token: authToken } });
        ...
        async 'deploy.model'(projectId, target, isTest = false) {
            ...
            const { gitSettings } = await Projects.findOne({ _id: projectId }, { fields: { gitSettings: 1 } });
            if (gitSettings) {
                try {
            ...
            const resp = isTest ? { status: 200, data: {} } : Meteor.call('axios.requestWithJsonBody', url, method, data, instance.token);
