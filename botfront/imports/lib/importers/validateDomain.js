...
export const deduplicateAndMergeResponses = (listOfResponse) => {
    ...
    return listOfResponse.reduce((all, resp) => {
        ...
        if (respSeen) {
            ...
            const respValueToAdd = resp.values.filter(
                val => !respSeen.values.includes(`${val.env}-${val.lang}`),
            );
            // add newly supported lang to the seen ones
            const newRespValues = respValueToAdd.map(val => `${val.env}-${val.lang}`);
            seen[key] = { values: [...respSeen.values, ...newRespValues], index: insertIndex };
            // update the final array
            const updatedResp = all[insertIndex];
            updatedResp.values.push(...respValueToAdd);
            return [
        ...
        const values = resp.values.map(val => `${val.env}-${val.lang}`);
        //  add those to the seen, as well as the index
        seen[key] = { values, index: all.length };
        // add the response
        return [...all, resp];
    }, []);
};
