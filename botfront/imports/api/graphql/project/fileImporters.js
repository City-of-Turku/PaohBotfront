...
import { indexBotResponse, getBotResponses, deleteResponse } from '../botResponses/mongo/botResponses';
...
const wipeDomain = async (projectId, deleteOnlyDevResponses = true) => {
    try {
        if (deleteOnlyDevResponses) {
            const responses = await getBotResponses(projectId);
            for (const response of responses) {
                await deleteResponse(projectId, response.key, 'development');
            }
        } else {
...
export const handleImportRasaConfig = async (files, { projectId, projectLanguages }) => {
    ...
    await files.reduce(async (previous, f) => {
        await previous;
        const {
            pipeline, default_for_multilang_nlu_model, policies, language,
        } = f;
        ...
        if (pipeline && !pipelineImported[language]) {
            try {
                if (existingLanguages.includes(language)) {
                    if (default_for_multilang_nlu_model) {
                        await Meteor.callWithPromise(
                            'nlu.update.pipeline',
                            projectId,
                            language,
                            safeDump({ default_for_multilang_nlu_model, pipeline }),
                        );
                    } else {
                        await Meteor.callWithPromise(
                            'nlu.update.pipeline',
                ...
                } else if (default_for_multilang_nlu_model) {
                    await Meteor.callWithPromise(
                        'nlu.insert',
                        projectId,
                        language,
