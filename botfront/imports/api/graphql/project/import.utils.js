...
export async function importSteps({
    ...
    if (onlyValidate) {
        // at botfront/imports/ui/components/settings/ImportRasaFiles.jsx we want to import NLU and Stories files separately
        // so we need to recognize NLU files and set the dataType to "nlu" because by default it's "training_data" for both NLU and Stories files
        if (filesAndValidationData.fileMessages[0].dataType === 'training_data' && filesAndValidationData.fileMessages[0].rawText.split(/(?<=(^|\n))(?=[a-z]+:)/).find(el => el.startsWith('nlu:\n'))) {
            filesAndValidationData.fileMessages[0].dataType = 'nlu';
        }
    ...
    return { summary: importResult.map(text => ({ text })), wipeInfos: { nluWiped: newParams.wipeNluData.length > 0, storiesWiped: newParams.wipeFragments } };
}
