        ...
        { $sort: sortObject },
        {
            $addFields: {
                'metadata.draft': { // clean up
                    // GraphQL + CosmosDB gave an error about $$REMOVE variable but works with $REMOVE
                    // (but this still works with Mongo too)
                    $cond: [{ $eq: ['$metadata.draft', false] }, '$REMOVE', '$metadata.draft'],
                },
            },
        },
    ]);
