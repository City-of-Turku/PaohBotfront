...
Meteor.methods({
    async 'stories.insert'(story) {
        ...
        const updateresult = Object.keys(storyGroups).map((_id) => {
    ...
    async 'stories.update'(story, options = {}) {
        ...
        const result = await Stories.update({ _id }, {
            $set: {
                type: originStory.type, ...update, events: newEvents, textIndex, // TODO: Potential issues?
            },
        });
    ...
    async 'stories.delete'(story) {
        ...
        const result = await StoryGroups.update(
