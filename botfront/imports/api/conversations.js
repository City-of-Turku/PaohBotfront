...
Meteor.startup(() => {
    if (Meteor.isServer) {
        ...
        // Conversations.index({ env: 1, 'tracker.latest_event_time': -1 }); // Compound index does not support nested paths
        Conversations._ensureIndex({ 'tracker.latest_event_time': -1 });
    }
});
