...
const AnalyticsDashboard = ({ dashboard, onUpdateDashboard }, ref) => {
    ...
    // Set card dates to show last 30 days on first render
    useEffect(() => {
        const newStartDate = moment().subtract(30, 'days').startOf('day');
        const newEndDate = moment().endOf('day');
        onUpdateDashboard({
            cards: cards.map(card => ({ ...card, startDate: newStartDate, endDate: newEndDate })),
        });
    }, []);
