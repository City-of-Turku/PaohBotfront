        ...
        const setFileListAfterRecompute = update => params.validateFunction(update).then((data, err) => {
            ...
            return setFileList({
                set: update.map((file, i) => {
                    ...
                    const dataType = data[i]?.dataType || file.dataType || [];
                    const skipImport = data[i]?.skipImport || false;
                   
                    return addDataToFile(file, {
                        validated: true, errors, warnings, dataType, skipImport,
                    });
                }),
            });
        });
