 ...
 */
    ...
    _loginUser(methodInvocation, userId, stampedLoginToken) {
        ...
        // connection).
        Meteor._noYieldsAllowed(() => this._setLoginToken(
            userId,
    ...
    _attemptLogin(
        ...
        let user;
        if (result.userId) user = this.users.findOne(result.userId, { fields: this._options.defaultFieldSelector });
    ...
    _runLoginHandlers(methodInvocation, options) {
        for (const handler of this._loginHandlers) {
    ...
    updateOrCreateUserFromExternalService(
        ...
        if (serviceName === 'password' || serviceName === 'resume') {
            throw new Error(
                `Can't use updateOrCreateUserFromExternalService with internal service ${serviceName}`,
            );
        }
        ...
        if (user) {
            ...
            let setAttrs = {};
            Object.keys(serviceData).forEach(key => setAttrs[`services.${serviceName}.${key}`] = serviceData[key]);
...
const defaultResumeLoginHandler = (accounts, options) => {
    ...
    // below 'fields' doesn't work with Cosmos
    // { fields: { 'services.resume.loginTokens.$': 1 } },
    );
